...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    bool pressed = false;
    bool hovered = ItemHoverable(bb, id, NULL);
...
bool ImGui::Selector(bool selected, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    static std::map<ImGuiID, tab_state2> anim;
    auto it_anim = anim.find(id);

    if (it_anim == anim.end())
    {
        anim.insert({ id, tab_state2() });
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.outline = ImLerp(it_anim->second.outline, IsItemActive() ? ImVec4(ImStyle::general_color) : ImStyle::button::outline, g.IO.DeltaTime * 6.f);
    ...
    ImGui::GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.outline), ImStyle::button::rounding);

    invisible_shadow(1.f, bb.Min, bb.Max, GetColorU32(ImStyle::general_color), 500);

    PushStyleVar(ImGuiStyleVar_Alpha, it_anim->second.delay_opticaly);
    PushClipRect(bb.Min, bb.Max, false);
    GetWindowDrawList()->AddShadowCircle(bb.Min + ImVec2(size_arg) / 2, 3 + it_anim->second.circle_size, GetColorU32(ImStyle::general_color), 3 + it_anim->second.circle_size, ImVec2(0, 0), 100);
    PopClipRect();
    PopStyleVar();

    ImGui::GetWindowDrawList()->AddText(ImVec2(bb.Min.x + (size_arg.x - CalcTextSize(label).x) / 2, bb.Max.y - CalcTextSize(label).y - (size.y - CalcTextSize(label).y) / 2), GetColorU32(ImStyle::text::text_active), label);
...
bool ImGui::Tab(const char* icon, const char* tabname, int* tab, int number, int total_tabs, float total_region_width, float alpha) 
{
    ...
    float available_width = total_region_width - 40.0f;
    float tab_width = available_width / total_tabs;
    float tab_height = 35.0f;

    ImVec2 cursor_pos = ImGui::GetCursorScreenPos();
    ImRect total_bb(cursor_pos, cursor_pos + ImVec2(tab_width, tab_height));
    ...
    bool is_active = (*tab == number);
    tabAnim.background_color = ImLerp(tabAnim.background_color, is_active ? ImColor(ImGui::GetColorU32(ImStyle::general_color)) : ImColor(20, 20, 20, 175), g.IO.DeltaTime * anim_speed);
    tabAnim.text_color = ImLerp(tabAnim.text_color, ImColor(255, 255, 255, 255), g.IO.DeltaTime * anim_speed);
    tabAnim.text_color_offset = ImLerp(tabAnim.text_color_offset, is_active ? ImColor(0.f, 0.f, 0.f, 0.0f) : ImColor(0.f, 0.f, 0.f, 0.5f), g.IO.DeltaTime * anim_speed);

    window->DrawList->AddRectFilled(total_bb.Min + ImVec2(0, tab_height - 3), total_bb.Max, ImGui::GetColorU32(tabAnim.background_color), 6.0f);
    window->DrawList->AddShadowRect(total_bb.Min - ImVec2(0, 3), total_bb.Max, ImGui::GetColorU32(tabAnim.background_color), 10.f, ImVec2(0, 5), 0, 12.0f);

    if (is_active) {
        float shimmer_height = tab_height * 0.6f;
        float shimmer_y_offset = (tab_height - shimmer_height) * 0.5f;

        static std::map<ImGuiID, float> shimmer_offset;
        static std::map<ImGuiID, float> shimmer_delay_timer;

        float& offset = shimmer_offset[id];
        float& delay_timer = shimmer_delay_timer[id];

        float shimmer_speed = 150.0f;
        float shimmer_start = -40.0f;
        float shimmer_end = tab_width + 30.0f;
        float shimmer_delay_duration = 1.6f; 

        if (offset < shimmer_end) {
            offset = ImLinearSweep(offset, shimmer_end, g.IO.DeltaTime * shimmer_speed);
            delay_timer = 0.0f; 
        }
        else {
            delay_timer += g.IO.DeltaTime;
            if (delay_timer >= shimmer_delay_duration) {
                offset = shimmer_start;
                delay_timer = 0.0f;
            }
        }

        ImVec2 start = total_bb.Min + ImVec2(offset, shimmer_y_offset);
        ImVec2 end = start + ImVec2(2.0f, shimmer_height);

        ImGui::PushClipRect(total_bb.Min, total_bb.Max, true);
        window->DrawList->AddShadowRect(start, end, ImColor(255, 255, 255, 125), 80.0f, ImVec2(0, 0));
        ImGui::PopClipRect();
    ...
    window->DrawList->AddText(total_bb.Min + ImVec2(15, (tab_height - ImGui::CalcTextSize(icon).y) * 0.5f), ImColor(255, 255, 255, 255) - ImGui::GetColorU32(tabAnim.text_color_offset), icon);
    ImGui::PopFont();

    ImGui::PushFont(poppins_reg);
    window->DrawList->AddText(
        total_bb.Min + ImVec2((tab_width - ImGui::CalcTextSize(tabname).x) * 0.5f, (tab_height - ImGui::CalcTextSize(tabname).y) * 0.5f),
        ImColor(255, 255, 255, 255) - ImGui::GetColorU32(tabAnim.text_color_offset),
        tabname
    );
    ImGui::PopFont();


    ImGui::SetCursorScreenPos(ImVec2(total_bb.Max.x, total_bb.Min.y));
    ImGui::PopStyleVar();
    return pressed;
}
...
struct check_state
{
    ...
    ImVec2 particle_pos[80];
    ImVec2 particle_speed[80];
    float particle_radius[80];
    float particle_alpha[80];
...
bool ImGui::Checkbox(const char* label, const char* description, bool* v)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float checkbox_height = 50.f;
    const float checkbox_padding_lr = 0.f; // left-right padding
    const float checkbox_width = ImGui::GetContentRegionAvail().x - 2 * checkbox_padding_lr;

    const float square_sz = 20.f;
    const ImVec2 pos = window->DC.CursorPos + ImVec2(checkbox_padding_lr, 0); // apply left padding
    const ImVec2 size(checkbox_width, checkbox_height);
    const ImRect rect(pos, pos + size);
    ImRect particle_bb(rect.Min - ImVec2(30, 30), rect.Max + ImVec2(30, 30));
    ...
    ImGui::ItemSize(ImVec2(size.x + 2 * checkbox_padding_lr, checkbox_height), 0.f); // reserve space including padding
    ...
    window->DrawList->AddRectFilled(rect.Min, rect.Max, ImGui::GetColorU32(it_anim->second.rect_color, 0.1f), ImStyle::button::rounding, 0);

    it_anim->second.rect_color = ImLerp(it_anim->second.rect_color, *v ? ImStyle::general_color : ImStyle::elements::background, g.IO.DeltaTime * anim_speed);
    ...
    if (*v) {
        ...
        for (int i = 0; i < 80; i++)
        {
            ...
            {
                it_anim->second.particle_pos[i] = ImVec2(rect.Min.x - 30, rect.Min.y + rect.GetHeight() / 2);
                it_anim->second.particle_speed[i] = ImVec2(random_float(-8.000f, 14.000f), random_float(-8.000f, 14.000f));
                it_anim->second.particle_radius[i] = random_float(1.000f, 2.300f);
                it_anim->second.particle_alpha[i] = random_float(0.300f, 1.000f) - it_anim->second.alpha_particles;
            }

            it_anim->second.particle_pos[i] += it_anim->second.particle_speed[i] * ImVec2(g.IO.DeltaTime * 8.f, g.IO.DeltaTime * 8.f);
            window->DrawList->AddCircleFilled(it_anim->second.particle_pos[i], it_anim->second.particle_radius[i], ImColor(ImStyle::general_color.x + (150.f / 255), ImStyle::general_color.y + (150.f / 255), ImStyle::general_color.z + (150.f / 255), it_anim->second.particle_alpha[i]));
            window->DrawList->AddShadowCircle(it_anim->second.particle_pos[i], it_anim->second.particle_radius[i], ImColor(ImStyle::general_color.x + (20.f / 255), ImStyle::general_color.y + (20.f / 255), ImStyle::general_color.z + (20.f / 255), 1.f), 35.f + it_anim->second.particle_radius[i], ImVec2(0, 0), 0, 20);
        }

        ImGui::PopClipRect();
    }

    static std::map<ImGuiID, float> scale_anim;
    float& scale = scale_anim[id];

    if (pressed && g.LastItemData.StatusFlags & ImGuiItemStatusFlags_ToggledSelection)
        scale = 0.0f;

    scale = ImLerp(scale, *v ? 1.0f : 0.0f, g.IO.DeltaTime * 10.0f);

    float ease_scale = ImClamp(ImSin(scale * IM_PI * 0.5f), 0.0f, 1.0f);

    // Bounce Y offset (only when turning true)
    float bounce_offset_y = *v ? -ImSin(scale * IM_PI) * 6.0f : 0.0f;

    const ImVec2 box_min = rect.Max - ImVec2(40, 40);
    const ImVec2 box_max = rect.Max - ImVec2(12, 12);

    window->DrawList->AddRect(box_min, box_max, ImGui::GetColorU32(ImStyle::elements::background_widget, 0.7f), 3.f);
    window->DrawList->AddRectFilled(box_min, box_max, ImGui::GetColorU32(ImStyle::elements::background_widget, 0.5f), 3.f);
    window->DrawList->AddShadowCircle(ImVec2(box_max.x - 13, box_max.y - 13), 3.f, ImGui::GetColorU32(ImStyle::white, state.alpha), 40.f, ImVec2(0, 0), 0, 20);

    ImGui::PushClipRect(box_min, box_max, true);
    float square_size = box_max.x - box_min.x;
    ImVec2 center = (box_min + box_max) * 0.5f;
    center.y += bounce_offset_y;

    if (*v || scale > 0.001f)
        ImGui::RenderCheckMark(
            window->DrawList,
            center - ImVec2(square_size * 0.275f, square_size * 0.275f) * ease_scale,
            ImGui::GetColorU32(ImStyle::white + ImVec4(40.f / 255, 40.f / 255, 40.f / 255, 40.f / 255), state.alpha),
            square_size * 0.55f * ease_scale
        );
    ...
    // Texts
    RenderTextColor(lexend_b, rect.Min + ImVec2(10, 0), rect.Max, ImGui::GetColorU32(ImStyle::elements::text_active), label, ImVec2(0.0, 0.2));
    RenderTextColor(lexend_reg, rect.Min + ImVec2(10, 0), rect.Max, ImGui::GetColorU32(ImStyle::text::text2), description, ImVec2(0.0, 0.8));
...
bool ImGui::Keybind(const char* label, const char* description, int* key)
{
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, g.ActiveId == id ? ImStyle::elements::text_active : hovered ? ImStyle::elements::text_hov : ImStyle::elements::text, ImGui::GetIO().DeltaTime * 6.f);

    ImGui::GetWindowDrawList()->AddRectFilled(rect.Min, rect.Max, ImGui::GetColorU32(ImStyle::elements::background, 0.5f), ImStyle::elements::rounding);

    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, ImGui::GetColorU32(ImStyle::elements::background + ImVec4(ImColor(10, 10, 10))), ImStyle::elements::rounding);

    RenderTextColor(lexend_reg, rect.Min + ImVec2(10, 0), rect.Max, ImGui::GetColorU32(ImStyle::elements::text), description, ImVec2(0.0, 0.8));

    RenderTextColor(lexend_b, rect.Min + ImVec2(10, 0), rect.Max, ImGui::GetColorU32(ImStyle::elements::text_active), label, ImVec2(0.0, 0.2));

    RenderTextColor(lexend_b, clickable.Min, clickable.Max, ImGui::GetColorU32(ImStyle::elements::text_active), buf_display, ImVec2(0.5f, 0.5f));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 item_size = ImVec2(ImGui::GetContentRegionAvail().x - 0.0f, 50.0f);
    const ImVec2 widget_pos = window->DC.CursorPos + ImVec2(0.f, 0.f);
    const ImRect frame_bb(widget_pos, widget_pos + item_size);
    ...
    // Slider bar: 10px padding from left and right inside widget
    const ImRect slider_bb(frame_bb.Min + ImVec2(10, 30), frame_bb.Max - ImVec2(10, 13));
    ...
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(ImRect(slider_bb.Min - ImVec2(8, 0), slider_bb.Max + ImVec2(8, 0)), id, data_type, p_data, p_min, p_max, format, flags, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);
    ...
    it_anim->second.grab_offset = ImLerp(it_anim->second.grab_offset, (grab_bb.Min.x + (grab_bb.Max.x - grab_bb.Min.x) / 2) - slider_bb.Min.x, g.IO.DeltaTime * 10.f);
    ...
    //Item Background
    GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, ImGui::GetColorU32(ImStyle::elements::background, 0.5f), ImStyle::elements::rounding);
    // Slider bar background
    window->DrawList->AddRectFilled(slider_bb.Min, slider_bb.Max, ImGui::GetColorU32(ImStyle::elements::background_widget, 0.5f), 2.f);

    // Slider progress
    window->DrawList->AddRectFilled(slider_bb.Min, slider_bb.Min + ImVec2(it_anim->second.grab_offset, slider_bb.Max.y - slider_bb.Min.y), GetColorU32(ImStyle::general_color), 2.f);

    // Particle animation in filled region
    PushClipRect(slider_bb.Min, slider_bb.Min + ImVec2(it_anim->second.grab_offset, slider_bb.Max.y - slider_bb.Min.y), true);
    for (int i = 0; i < 50; i++)
    {
        if (!frame_bb.Contains(it_anim->second.particle_pos[i]))
        {
            it_anim->second.particle_pos[i] = slider_bb.Min + ImVec2(frame_size.x / 2, -10);
            ...
            it_anim->second.particle_alpha[i] = random_float(0.000f, 1.000f);
        }
        it_anim->second.particle_pos[i] += it_anim->second.particle_speed[i] * ImVec2(g.IO.DeltaTime * anim_speed, g.IO.DeltaTime * anim_speed);
        window->DrawList->AddShadowCircle(it_anim->second.particle_pos[i], it_anim->second.particle_radius[i], ImColor(ImStyle::general_color.x + (85.f / 255), ImStyle::general_color.y + (85.f / 255), ImStyle::general_color.z + (85.f / 255), 1.f), 35.f + it_anim->second.particle_radius[i], ImVec2(0, 0), 0, 20);
    }
    PopClipRect();

    // Draw label (top-left)
    ImGui::PushFont(lexend_b);
    window->DrawList->AddText(frame_bb.Min + ImVec2(10, 5), GetColorU32(it_anim->second.text_color), label);
    ImGui::PopFont();

    // Draw value (top-right)
    ImGui::PushFont(lexend_b);
    window->DrawList->AddText(ImVec2(frame_bb.Max.x - 10 - CalcTextSize(value_buf).x, frame_bb.Min.y + 5), GetColorU32(it_anim->second.text_color), value_buf);
    ImGui::PopFont();
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (!is_multiline)
    {
        ...
        it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? ImStyle::input::background_active : hovered ? ImVec4(ImStyle::general_color.x, ImStyle::general_color.y, ImStyle::general_color.z, 205 / 255.f) : ImVec4(ImStyle::general_color.x, ImStyle::general_color.y, ImStyle::general_color.z, 255 / 255.f) , g.IO.DeltaTime * 6.f);
        it_anim->second.shadowcol = ImLerp(it_anim->second.shadowcol, IsItemActive() ? ImStyle::input::shadow_active : hovered ? ImVec4(ImStyle::general_color.x, ImStyle::general_color.y, ImStyle::general_color.z, 205 / 255.f) : ImVec4(ImStyle::general_color.x, ImStyle::general_color.y, ImStyle::general_color.z, 255 / 255.f), g.IO.DeltaTime * 6.f);
        it_anim->second.shadow = ImLerp(it_anim->second.shadow, IsItemActive() ? 25.f : 10.f, g.IO.DeltaTime * 6.f);
        ...
        GetWindowDrawList()->AddShadowRect(frame_bb.Min + ImVec2(0, size_arg.y - 2), frame_bb.Max, GetColorU32(it_anim->second.shadowcol), it_anim->second.shadow, ImVec2(0, 0), ImStyle::button::rounding);
    }
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    {
        // Render SV Square
        draw_list->AddRectFilledMultiColor(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), col_white, hue_color32, hue_color32, col_white,0,0);
        draw_list->AddRectFilledMultiColor(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), 0, 0, col_black, col_black,0,0);
        ...
        // Render Hue Bar
        for (int i = 0; i < 6; ++i)
            draw_list->AddRectFilledMultiColor(ImVec2(bar0_pos_x, picker_pos.y + i * (sv_picker_size / 6)), ImVec2(bar0_pos_x + bars_width, picker_pos.y + (i + 1) * (sv_picker_size / 6)), col_hues[i], col_hues[i], col_hues[i + 1], col_hues[i + 1],0,0);
    ...
    if (alpha_bar)
    {
        ...
        draw_list->AddRectFilledMultiColor(bar1_bb.Min, bar1_bb.Max, user_col32_striped_of_alpha, user_col32_striped_of_alpha, user_col32_striped_of_alpha & ~IM_COL32_A_MASK, user_col32_striped_of_alpha & ~IM_COL32_A_MASK, 0,0);
